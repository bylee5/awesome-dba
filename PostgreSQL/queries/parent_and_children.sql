SELECT DISTINCT n1.nspname || '.' || p.relname AS parent,
                n2.nspname || '.' || c.relname AS child
--              , cstr.conname
FROM pg_class c
JOIN pg_constranint cstr ON (cstr.conrelid = c.oid AND c.relkind = 'r' AND cstr.contype = 'f')
