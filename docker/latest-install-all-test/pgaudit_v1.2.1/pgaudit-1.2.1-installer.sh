...
# install pgaudit
TAR=pgaudit-1.2.1.tar.gz
TAR_DIR=pgaudit-1.2.1
...
if [ $? -ne 0 ]; then
    echo -e "${RED}Failed: Install pgaudit \"$work_home$TAR_DIR\"${NC}"
    exit
fi
...
if [ 0 -eq $shared_preload_libraries_line_count ]; then
	cat $AGDATA/postgresql.conf | sed "s/#shared_preload_libraries = 'pg_stat_statements,pg_statsinfo,pg_hint_plan,hll'/shared_preload_libraries = 'pg_stat_statements,pgaudit'/" > $AGDATA/postgresql.conf_modified
	if [ $? -ne 0 ]; then
	...
	cat $AGDATA/postgresql.conf | sed "s/shared_preload_libraries = '${shared_preload_library_list}'/shared_preload_libraries = '${shared_preload_library_list},pgaudit'/" > $AGDATA/postgresql.conf_modified
	if [ $? -ne 0 ]; then
    	echo -e "${RED}Failed: Modify postgresql.conf${NC}"
    	exit
	fi
...
echo -e "** Completed successfully pgaudit."
